...
getEverythingInScope metaVar = do
  ...
  let nameSpace = Scope.everythingInScope scope
      ...
      qnames = fmap (Scope.anameName . NonEmptyList.head) $ Map.elems names
  return qnames
...
refine branch = withBranchState branch $ do
  ...
  withBranchAndGoal branch1 goal1 $ do
    goalType1 <- bench [Bench.Deserialization, Bench.Reduce] $ reduce =<< getMetaTypeInContext (goalMeta goal1)
...
assignMeta metaId term metaType = bench [Bench.Deserialization, Bench.CheckRHS] $
  metasCreatedBy (do
    metaVar <- lookupLocalMeta metaId
    metaArgs <- getMetaContextArgs metaVar
